...
public class DeactivateFoodItemUseCase implements DeactivateFoodItemInput {

    private final FoodItemServiceHelper foodItemServiceHelper;

    public DeactivateFoodItemUseCase(FoodItemServiceHelper foodItemServiceHelper) {
    ...
    public void execute(UUID restaurantId, UUID foodItemId) {
        ...
        if (!foodItem.isActive()) {
            throw new FoodItemOperationNotAllowedException(FOOD_ITEM_IS_ALREADY_INACTIVE);
        }
